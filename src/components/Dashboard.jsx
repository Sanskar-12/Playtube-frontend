...
const Dashboard = () => {
  ...
  channelData?.videos.forEach((video) => {
    totVideoViews += video?.views?.length || 0;
  });

  let totShortViews = 0;
  channelData?.shorts.forEach((short) => {
    totShortViews += short?.views?.length || 0;
  });

  const totalViews = totShortViews + totVideoViews;
            ...
            {channelData?.shorts
              ...
              .map((s, idx) => (
                <ContentCard1
                  key={idx}
                  content={s}
                  onClick={() => navigate(`/watch-short/${s?._id}`)}
                />
              ))}
          </div>
        </div>
...
const AnalyticsCard = ({ label, value, onClick }) => {
  return (
    <div
...
const ContentCard = ({ content, isShort = false, onClick }) => {
  return (
    <div
    ...
    >
      ...
      <img
        ...
        className={`${
          isShort
            ? "w-full sm:w-28 h-48 sm:h-40"
            : "w-full sm:w-40 h-48 sm:h-24"
...
const ContentCard1 = ({ content, onClick }) => {
  return (
    <div
      className="flex flex-col sm:flex-row gap-4 items-start bg-[#0f0f0f] border border-gray-700 rounded-lg p-3 sm:p-4 hover:bg-[#202020] transition"
      onClick={onClick}
